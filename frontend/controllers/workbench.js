(function exposeWorkbench(global) {
  "use strict";

  function mount(root, userConfig = {}) {
    if (!root || typeof root.querySelector !== "function") {
      throw new TypeError("LitexChessWorkbench.mount requires a Document, Element, or ShadowRoot");
    }
    ...
    const doc = root.ownerDocument || root.host?.ownerDocument || document;
      ...
      const textbookBase = String(config.textbookBase || "/textbook/Chess").replace(/\/$/, "");
      ...
      const textbookUrl = (suffix = "") => `${textbookBase}${suffix ? `/${String(suffix).replace(/^\//, "")}` : ""}`;
    ...
    function renderLessonContext(context) {
      ...
      if (!context) {
        card.hidden = true;
        byId("textbookLink").href = textbookBase;
        return;
      }
      card.hidden = false;
      ...
      const returnUrl = context.returnUrl || textbookBase;
      byId("lessonContextReturn").href = returnUrl;
      byId("textbookLink").href = returnUrl;
    }

    async function resolveWorkbenchContext() {
      const params = new URLSearchParams((config.search != null ? config.search : win.location.search));
      ...
      const context = {
        ...
        returnUrl: params.get("return") || textbookBase,
      };
      if (!fen && context.lab) {
        try {
          ...
          context.returnUrl = params.get("return") || lab.textbook_url || textbookBase;
        } catch (error) {
    ...
    async function onDrop(event) {
      ...
      if (Math.abs(code) === 1 && (to[1] === "1" || to[1] === "8")) {
        const answer = win.prompt("升变为 q / r / b / n", "q");
    ...
    function renderTree() {
      ...
      const titlebar = create("div");
      titlebar.className = "notation-titlebar";
      const matchup = create("div");
      ...
      const rootButton = create("button");
      ...
      let blocks;
      try {
        blocks = (config.notation || globalThis.LitexChessNotation).buildNotationBlocks(state.tree);
      } catch (error) {
        const failure = create("div");
    ...
    async function loadExport() {
      try {
        const response = await fetch(resolveApi(`/api/sessions/${state.sessionId}/export-pgn`));
    ...
    function wireEvents() {
      ...
      all(".formal-code-tab").forEach((button) => {
  ...
  global.LitexChessWorkbench = Object.freeze({ mount });
