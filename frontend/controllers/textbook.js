(function exposeTextbook(global) {
  "use strict";

  function mount(root, userConfig = {}) {
    if (!root || typeof root.querySelector !== "function") {
      throw new TypeError("LitexChessTextbook.mount requires a Document, Element, or ShadowRoot");
    }
    const config = { ...userConfig };
    const doc = root.ownerDocument || root.host?.ownerDocument || global.document;
    const win = doc?.defaultView || global;
    const byId = (id) => (typeof root.getElementById === "function" ? root.getElementById(id) : root.querySelector(`#${id}`));
    const all = (selector) => root.querySelectorAll(selector);
    const create = (tag) => doc.createElement(tag);
    const apiBase = String(config.apiBase || "/api").replace(/\/$/, "");
    const sourceBase = String(config.sourceBase || "/textbook-source").replace(/\/$/, "");
    const chapterBase = String(config.chapterBase || "/textbook/Chess").replace(/\/$/, "");
    const workbenchBase = String(config.workbenchBase || `${chapterBase}/workbench`).replace(/\/$/, "");
    const resolveUrl = (url) => {
      if (/^https?:\/\//i.test(url)) return url;
      let value = String(url || "");
      if (value === "/api") return apiBase;
      if (value.startsWith("/api/")) return `${apiBase}/${value.slice(5)}`;
      if (value === "/textbook-source") return sourceBase;
      if (value.startsWith("/textbook-source/")) return `${sourceBase}/${value.slice(17)}`;
      return value;
    };
    const chapterUrl = (slug) => `${chapterBase}/${encodeURIComponent(slug)}`;
...
function endgameWorkbenchUrl(lesson, fen = null, hint = "") {
  ...
  const params = new URLSearchParams({
    ...
    return: chapterUrl(currentChapter()?.slug || "interactive-endgames"),
  });
  if (hint) params.set("hint", hint);
  return `${workbenchBase}?${params.toString()}`;
}
...
function renderChapter() {
  ...
  if (!chapter) return;
  if (config.manageDocumentTitle !== false) doc.title = `第 ${chapter.number} 章 · ${chapter.title} — Litex 国际象棋规则教材`;
  ...
  all(".chapter-link").forEach((button, index) => {
...
function selectChapter(index, notify = true) {
  const chapter = state.catalog?.chapters?.[index];
  if (!chapter) return;
  state.chapterIndex = index;
  renderChapter();
  if (notify) {
    if (typeof config.onChapterChange === "function") {
      config.onChapterChange(chapter, { index, url: chapterUrl(chapter.slug) });
    } else if (config.manageHash !== false) {
      win.history.replaceState(null, "", `#${chapter.slug}`);
    }
  }
  if (config.manageScroll !== false) win.scrollTo({ top: 0, behavior: "smooth" });
  else root.host?.scrollIntoView?.({ block: "start" });
}

function setBusy(busy) {
  state.busy = busy;
  byId("runAllButton").disabled = busy;
  all(".run-example, .lab-move-button, .candidate-square, .run-status-lab, .run-history-lab, .start-endgame, [data-endgame-action]").forEach((button) => {
...
async function runBoardMove(labId, moveId) {
  ...
  try {
    ...
    const target = byId(`lab-${labId}`)?.querySelector(".board-lab-result");
...
async function runStatusLab(labId) {
  ...
  try {
    ...
    byId(`status-lab-${labId}`)?.querySelector(".status-lab-result")?.scrollIntoView({ behavior: "smooth", block: "nearest" });
...
async function runHistoryLab(labId) {
  ...
  try {
    ...
    byId(`history-lab-${labId}`)?.querySelector(".history-lab-result")?.scrollIntoView({ behavior: "smooth", block: "nearest" });
...
async function initialize() {
  try {
    const [catalog, source, status, endgames] = await Promise.all([
      ...
      fetch(resolveUrl("/textbook-source/chess_rules_textbook_cn.lit")).then((response) => {
    ...
    const requested = String(config.initialChapter || (config.manageHash !== false ? win.location.hash.replace(/^#/, "") : ""));
...
const onReset = () => resetResults();
const onPrevious = () => selectChapter(state.chapterIndex - 1, true);
const onNext = () => selectChapter(state.chapterIndex + 1, true);
byId("runAllButton").addEventListener("click", onRunAll);
byId("resetButton").addEventListener("click", onReset);
byId("previousChapter").addEventListener("click", onPrevious);
byId("nextChapter").addEventListener("click", onNext);

const onHashChange = () => {
  if (!state.catalog || config.manageHash === false) return;
  const slug = win.location.hash.replace(/^#/, "");
...
if (config.manageHash !== false) win.addEventListener("hashchange", onHashChange);

const ready = initialize();
return {
  ready,
  state,
  selectChapterBySlug(slug, notify = false) {
    const index = state.catalog?.chapters?.findIndex((chapter) => chapter.slug === slug) ?? -1;
    if (index < 0) return false;
    selectChapter(index, notify);
    return true;
  },
  currentChapter,
  destroy() {
    if (config.manageHash !== false) win.removeEventListener("hashchange", onHashChange);
    byId("runAllButton")?.removeEventListener("click", onRunAll);
    byId("resetButton")?.removeEventListener("click", onReset);
    byId("previousChapter")?.removeEventListener("click", onPrevious);
    byId("nextChapter")?.removeEventListener("click", onNext);
  },
};

  }

  global.LitexChessTextbook = { mount };
})(globalThis);
